<svelte:options customElement={{ tag: 'lut-lut-editor-tab', shadow: 'none' }} />
...
<script lang="ts">
  ...
  import PreviewPane from '../lut-editor-dialog/lut-editor-preview-pane.svelte';
  import RampSection from '../lut-editor-dialog/lut-editor-ramp-section.svelte';
  import StopSection from '../lut-editor-dialog/lut-editor-stop-section.svelte';
  import { createPointerSessionManager } from '../lut-editor-dialog/pointer-session.ts';
  import {
      ...
      serializeRampData,
  } from '../lut-editor-dialog/shared.ts';
  ...
  let previousLutId: string | null = null;
  let previousSyncKey = '';
  ...
  $effect(() => {
    ...
    const nextLutId = lutId ?? null;
    const isNewSession = nextLutId !== previousLutId;
    previousLutId = nextLutId;

    editingLutId = nextLutId;
    editorRampData = rampData;

    // 初期スナップショットは LUT タブ開始時のみ更新する。
    // 編集ごとの rampData 同期で更新すると isDirty が常に false になる。
    if (isNewSession || initialSerializedRampData === '') {
  ...
  /**
   * 内部状態を更新し、lut-change イベントを発行する（リアルタイムプレビュー用）。
   */
  function setNextRampData(nextData: ColorRamp2dLutData): void {
    editorRampData = nextData;
    previewRevision += 1;
    dispatch('lut-change', { lutId: editingLutId, rampData: nextData });
  }
...
<div class="root">
  <div class="head">
    ...
    <div class="head-actions">
      <Button variant="submit" handlePress={handleApply} disabled={!editorRampData || !isDirty}>
        {tr('lutEditor.apply')}
      </Button>
    </div>
  </div>
